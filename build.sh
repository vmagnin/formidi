...
# Compiling modules:
"${FC}" ${flags} -c src/MIDI_file_class.f90 src/music_common.f90 src/music.f90 src/GM_instruments.f90  src/MIDI_control_changes.f90

# Compiling examples:
for file in "third_kind" "canon" "blues" "circle_of_fifths" "la_folia" ; do
  echo "${file}"
  "${FC}" ${flags} MIDI_file_class.o music_common.o music.o  GM_instruments.o  MIDI_control_changes.o example/${file}.f90 -o build/${file}.out
done
